...
function addBrick(w, l, flat=false, pos=null) {
  ...
  const brickHeight = flat ? cellSize/4 : cellSize/2;

  const geo = new THREE.BoxGeometry(w*cellSize, brickHeight, l*cellSize);
  const baseMat = new THREE.MeshStandardMaterial({ color: 0xff3333 });
  const materials = [];
  for (let i = 0; i < 6; i++) materials.push(baseMat.clone());
