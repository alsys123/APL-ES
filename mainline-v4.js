...
async function loadQuestionsFromCSV(questionsFromCSV,sections,metadata) {
	...
	const qId = parseInt(String(row[0]).replace(/[^\d]/g, ""), 10);
    
          //  const section = parseInt(row[1]);
	const section = parseInt(String(row[1]).replace(/[^\d]/g, ""), 10);

//              const part = parseInt(row[2]);
	const part = parseInt(String(row[2]).replace(/[^\d]/g, ""), 10);
	    ...
	    //  console.log("shuffle map", shuffleMap[qId]);
	     ...
	     // console.log("in loop : questions =", JSON.stringify(questions, null, 2));
...
function getStatusClass(correct, incorrect, unanswered, total, section, part) {
    ...
    if (gExamType === 'student') {
	if (unanswered === 0) return 'doneGray5';
	if (unanswered === 1) return 'doneGray4';
	if (unanswered === 2) return 'doneGray3';
	if (unanswered === 3) return 'doneGray2';
	if (unanswered === 4) return 'doneGray1';
